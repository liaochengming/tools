...
void md5_encode(const uint8_t *initial_msg, size_t initial_len, uint8_t *digest) {
...
std::string get_md5(std::string input)
{
	...
	for (i = 0; i < 1000; i++)
	{
		md5_encode((uint8_t*)msg, len, result);
	}

	for (i = 0, k = 0; i < 16; i++)
	{
		//printf("%2.2x", result[i]);
		sprintf(&mystr[k], "%2.2x", result[i]);
		k = k + 2;
	}

	std::string out_str = mystr;
